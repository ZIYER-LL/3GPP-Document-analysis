...
- 调用任务解析工具，识别用户希望分析的会议、Agenda Item 和任务类型；
- 调用文稿检索工具，在 TDoc 清单中定位目标文稿；
- 调用下载与解压工具，自动获取并展开原始文稿文件；
- 调用正文提取工具，将 PDF / DOCX / TXT / MD 转换为可分析文本；
- 调用本地模型服务，生成单文稿摘要和多文稿对比分析；
- 调用报告生成工具，输出单篇报告、Agenda 总报告和前端展示结果；
- 调用任务状态管理工具，持续更新异步任务进度，便于用户在前端跟踪执行过程。
